...
pub fn handle_player_input(
    ...
    if let Ok(mut velocity) = query.get_single_mut() {
        ...
        if keyboard_input.pressed(KeyCode::KeyW) { // Use KeyW
            velocity.y += 1.0;
        }
        if keyboard_input.pressed(KeyCode::KeyS) { // Use KeyS
            velocity.y -= 1.0;
        }
        if keyboard_input.pressed(KeyCode::KeyA) { // Use KeyA
            velocity.x -= 1.0;
        }
        if keyboard_input.pressed(KeyCode::KeyD) { // Use KeyD
            velocity.x += 1.0;
        }
...
pub fn apply_velocity(
    mut query: Query<(&mut Transform, &PlayerVelocity, &PlayerSpeed), With<Player>>,
    time: Res<Time>,
) {
